...
export function DetraccionesModal({ companyId, companyName, year, ladoInicial = 'cobradas', onClose }: {
  ...
  const estadoColor = (e: string) => e === 'Completo' ? { bg: 'rgba(16,185,129,0.12)', fg: 'var(--green)' }
    : e === 'Pendiente' ? { bg: 'rgba(239,68,68,0.12)', fg: 'var(--red)' }
    : { bg: 'rgba(245,158,11,0.12)', fg: 'var(--yellow)' };
  ...
  const ladoBtn = (l: Lado, label: string) => (
    <button onClick={() => setLado(l)} style={{
      ...
      border: lado === l ? '1px solid rgba(43,180,187,0.5)' : '1px solid var(--modal-border-2)',
      background: lado === l ? 'rgba(43,180,187,0.2)' : 'var(--modal-input-bg)',
      color: lado === l ? 'var(--primary-light)' : 'var(--text-muted)',
    }}>{label}</button>
  );
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 1200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }}
      ...
      <div ref={modalRef} role="dialog" aria-modal="true" tabIndex={-1}
        ...
        style={{ background: 'var(--modal-bg)', border: '1px solid var(--modal-border-1)', borderRadius: '0.75rem', maxWidth: '96vw', width: 1180, maxHeight: '88vh', overflow: 'auto', padding: '1.5rem', outline: 'none' }}
        onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
          <div>
            <div style={{ fontWeight: 700, fontSize: '1rem', color: 'var(--text-primary)' }}>Reporte de Detracciones</div>
            <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginTop: '0.2rem' }}>
              {companyName ? `${companyName} · ` : ''}{year} · {lado === 'cobradas' ? 'Cobradas (facturas emitidas)' : 'Pagadas (facturas recibidas)'} · {filtered.length} doc · Detracción total: <b style={{ color: 'var(--yellow)' }}>{fmt(totalDetrac)}</b>
            </div>
          </div>
        ...
        <div style={{ display: 'flex', gap: '0.5rem', marginBottom: '1rem', flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <label style={{ fontSize: '0.72rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>Desde
            <input type="date" value={desde} min={`${year}-01-01`} max={`${year}-12-31`} onChange={e => setDesde(e.target.value)}
              style={{ padding: '0.3rem 0.4rem', borderRadius: 6, fontSize: '0.74rem', background: 'var(--modal-input-bg)', border: '1px solid var(--modal-border-3)', color: 'var(--text-primary)', colorScheme: 'dark' }} /></label>
          <label style={{ fontSize: '0.72rem', color: 'var(--text-muted)', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>Hasta
          ...
          }} style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', padding: '0.3rem 0.7rem', borderRadius: '0.4rem', cursor: 'pointer', fontSize: '0.72rem', fontWeight: 600, border: '1px solid var(--modal-border-2)', background: 'var(--modal-input-bg)', color: 'var(--text-muted)' }}>
        ...
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '2rem', color: 'var(--text-muted)' }}>Sin documentos con detracción en este período.</div>
        ) : (
            ...
            <table className="table-s10" style={{ fontSize: '0.76rem' }}>
                ...
                {filtered.map((r: any, i: number) => {
                  ...
                  return (
                    <tr key={i}>
                      ...
                      <td style={{ textAlign: 'right', color: 'var(--yellow)', fontWeight: 600 }}>{fmt(r.montoDetraccion)}</td>
                      <td style={{ whiteSpace: 'nowrap', color: r.fechaDetraccion ? 'var(--text-primary)' : '#6B7280' }}>
                      ...
                      <td style={{ textAlign: 'right', color: 'var(--text-muted)' }}>{(r.montoPago ?? 0) > 0 ? fmt(r.montoPago) : '—'}</td>
                      <td style={{ whiteSpace: 'nowrap', color: r.fechaPago ? 'var(--text-primary)' : '#6B7280' }}>{r.fechaPago || '—'}</td>
                    </tr>
                  );
                })}
              </tbody>
              <tfoot>
                <tr className="total-row">
                  ...
                  <td style={{ textAlign: 'right', color: 'var(--yellow)' }}>{fmt(totalDetrac)}</td>
                  <td colSpan={3} />
                </tr>
              </tfoot>
            </table>
